...
pkgdatadir = $(datadir)/stonecore
pkgincludedir = $(includedir)/stonecore
pkglibdir = $(libdir)/stonecore
pkglibexecdir = $(libexecdir)/stonecore
...
CONFIG_HEADER = $(top_builddir)/src/config/stone-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/stone-config.h.in \
...
ACLOCAL = ${SHELL} /root/projects/stone2/build-aux/missing aclocal-1.16
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /root/projects/stone2/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/projects/stone2/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/projects/stone2/build-aux/missing automake-1.16
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = 
BITCOIN_CLI_NAME = stone-cli
BITCOIN_DAEMON_NAME = stoned
BITCOIN_GUI_NAME = stone-qt
BITCOIN_TX_NAME = stone-tx
...
COPYRIGHT_HOLDERS_FINAL = The Stone Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = Stone Core
COPYRIGHT_YEAR = 2018
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /root/projects/stone2/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = stonecore
PACKAGE_BUGREPORT = https://github.com/stone/stone/issues
PACKAGE_NAME = Stone Core
PACKAGE_STRING = Stone Core 2.1.0
PACKAGE_TARNAME = stonecore
PACKAGE_URL = 
...
ZMQ_LIBS = -lzmq
abs_builddir = /root/projects/stone2
abs_srcdir = /root/projects/stone2
abs_top_builddir = /root/projects/stone2
abs_top_srcdir = /root/projects/stone2
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/projects/stone2/build-aux/install-sh
...
BITCOIND_BIN = $(top_builddir)/src/stoned$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/stone-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/stone-cli$(EXEEXT)
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_stone_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_stone_coverage.info test_stone.info
...
src/config/stone-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/stone-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/stone-config.h
$(top_srcdir)/src/config/stone-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/stone-config.h src/config/stamp-h1
...
#test_stone.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_stone -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_stone_filtered.info: test_stone.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_stone_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_stone_filtered.info
...
#test_stone_coverage.info: baseline_filtered_combined.info test_stone_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_stone_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_stone_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_stone_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_stone.coverage/.dirstamp:  test_stone_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_stone.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_stone.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
